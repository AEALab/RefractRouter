...
RefractAgent 规划路由的 Static 模式接入与功能测试已获用户授权连续执行，
不再逐项要求事先限定测试范围或确认 AFP 预算；此授权只适用于 Static。
每次真实调用仍记录实际模型、用量、费用、结果及错误，并保留零 HTTP 自动重试、
未知用量停止和运行证据。插件内的单任务额度属于运行配置，由设置页管理；
本段授权不自动修改该配置。其他规划模式的付费测试仍按本节一般规则办理。
...
- 安装前核对用户实际安装版本及已有功能来源，不能只与当前主分支比较；特别检查尚未合入的已安装开发分支。
- 客户端发布须同时保留 `conversation.view` 的任务 DAG 页签与 `settings.plugin.item` 的配置卡片；运行共同注册、历史回放和动态拓扑回归测试。
- 将源码、构建包、已安装包、运行界面分开验收。测试通过只证明测试覆盖的实现；声称恢复须确认实际包版本、服务加载和浏览器中节点及连线可见。
- UI 恢复不得顺带改变额度、预算、模型策略或思考默认值；其他运行故障单独记录，不用不断扩大限制代替诊断。
- 自动审批超时是基础设施结果，不是用户漏批，也不是安全拒绝。按工具允许最多重试一次；相同阻塞跨轮累计，无新证据或状态变化不得重置重试计数。
- 超时后先检查命令是否执行、已安装状态及官方 CLI 的实际行为；只有核对副作用和既有授权后，才考虑范围更窄的替代操作，不绕过明确的安全拒绝。
- 替代操作一次成功只能证明该次路径可用，不能宣称已定位审批系统内部根因。无新信息时不反复发送相同审批说明消耗用户时间和额度。
- 具体事故、证据与尚未确认的原因见 [DAG 恢复核对](docs/dag-restoration-audit.md)。
