...
- RefractAgent 模型入口通过已安装的 Python 核心执行，可脱离源码目录运行；
  历史验证和文本任务工具仍依赖匹配的源码环境。文档须区分这些入口，
  团队集中 Router 服务仍属于后续部署目标。
...
这项授权不表示无限调用额度，也不取消底层客户端零重试、版本冻结与证据保留要求。
用户已授权实现节点失败后的有界模型切换。应用层通过 `maxNodeFallbacks` 显式开启，
每节点至多切换两次；它不同于底层 HTTP 自动重试，全部尝试须记账和归档。
历史零回退协议保持原样。新增收益实验须冻结回退规则、次数与调用包络；不得把启用
局部切换后已变成异构分配的路线继续标为「整图单模型」。
