...
估算合同与审计字段见 [v4 两级自动路由选择器](automatic-routing-v4.md)。DSH 设置卡片已提供
路由目标、安全模式、Provider、模型与信任策略的结构化编辑，并显示职责和部署清单的结构预览；
预览不复制 Python 的安全准入或路由判定，高级 JSON 继续支持完整配置的无损往返。运行期逐阶段
重新分级、实际账本回填和预测误差校准仍属于 Issue #116 的后续工作。v4 已允许
