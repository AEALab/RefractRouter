...
思考token计入输出配额时，规划器可能在输出完整JSON之前耗尽上限。核心0.4.4仅对
Ark官方端点上的豆包mini紧凑规划副本，在没有显式thinking或reasoning_effort时默认
关闭思考。节点执行、评审、其他型号、其他端点、完整规划以及用户显式思考设置不被覆盖。

规划选择依据记录该默认策略。输出上限保持不变，截断计划仍然失败；错误诊断显示结束原因、
输出token、思考token和输出上限。确定性模拟测试验证阶段隔离、显式设置保留及截断失败行为。
