...
输入、缓存输入、输出价格和说明；质量与时延不能由用户填写。质量必须来自公开、独立第三方
评测的冻结先验，时延由同一 `(provider, model)` 路线的正常调用自动积累。没有时延样本时
使用明确标记的保守冷启动预测，不会为了测时延额外发起付费调用。

模型池 v2 的价格优先级为“用户价格覆盖 > 冻结公开价格 > 不可用”。旧 v1 配置仍可读取，
但其中手工填写的质量与时延不会成为运行证据；设置页会要求显式迁移。没有合规第三方质量
先验的路线可以保存为草稿，但不能参与路由。

本地运行的时延观测保存在 `runsDir/route-observations.sqlite3`；团队 HTTP v2 复用服务配置的
SQLite，并按项目隔离。只将正常结束且账本状态为 `billed` 的调用纳入最近 50 个成功样本的
p90 预测；失败、超时和取消保留状态计数但不进入成功时延。身份包含 provider、模型、有效
模型版本和推理档位，同名模型不会跨 provider 共享。`preflight` 与 `demo` 只读取该库，不写入
虚假观测；`refractagent route-profiles --runs-dir ...` 可以零调用查看当前快照。

产品开发测试可以在高级设置中显式启用“允许同一路线兼任执行与评审”。此时单一路线可同时
承担规划、执行、评审和分类；核心会在 `model_profile_provenance` 写入最终职责，并将
`independent_judge` 标为 `false`。该开关不会伪造独立评审，也不能作为研究结论或 #112 的
本地模型验收证据。未启用时，执行与评审仍必须使用不同路线。路线的部署属性同样由用户声明，
证据只记录声明，不把云端模型自动认定为物理本地模型。

当前公开档案位于 `data/model-profiles-v2.json`；`data/model-profiles-v1.json` 仅用于兼容已经
保存的旧路线。v2 除来源 URL、抓取时间、指标版本与归一化说明外，还保存条件价格计划：
DeepSeek 的峰谷价、MiniMax 的上下文与服务等级阶梯，以及 Kimi 独立的缓存写入费用。
...
和模拟；开发试用 `auto-live` 还要求 `synthetic`、CNY（人民币）生产／评审费用选择与设置级显式启用，且固定使用
...
容量、DAG 总输出上限和已设置的单任务人民币费用硬上限仍生效。生产与评审金额可分别
选择“无限制”；这不是默认值，启用后该类别可能发生实际费用。预检摘要把该类硬上限
记为 `null`，并记录 `production_unlimited` 或 `evaluation_unlimited`，两种选择均参与授权指纹。
