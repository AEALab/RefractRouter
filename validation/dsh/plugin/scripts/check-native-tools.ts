...
assert.ok(session.snapshotEvents().every(e=>KNOWN_SESSION_EVENT_TYPES.has(e.type)||e.ignorable===true))
assert.equal(session.snapshotEvents().some(e=>e.type==='tool/call'||e.type==='tool/result'),false)
