...
Python 核心在每次运行前根据宿主解析的只读目录快照、冻结公开档案和用户覆盖分配规划模型、
评审模型、分类模型与执行模型池。高级设置可以覆盖职责、输入／缓存输入／输出价格、质量、
时延和说明，也可以清空覆盖恢复公开档案。未知路线必须补齐这些预测后才能参与路由，证据中
会标记为“用户声明、未经项目校准”。旧 `providerConfig` 仅作为迁移来源保留，不会被静默覆盖。

当前版本仍不启用 v4 真实付费执行；目录配置、自动预览和模拟运行不调用真实模型。
