...
默认 `auto` 仍只做模拟。设置页显式启用实验性真实执行、选择 `synthetic`、填写生产与评审 USD
硬上限后，才显示 `auto-live`。每个任务先运行零调用复杂度与预算预检，再请求 DSH 一次性审批；
拒绝时不解析凭证、不派发模型。首版不传递工具，不启用回退、动态拆分、并发或远程 Router live。
