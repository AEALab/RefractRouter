...
def main():
    ...
    parser.add_argument('--bindings', type=Path,
                        default=ROOT / 'data/research/security-benchmark-bindings-v1.json')
    parser.add_argument('--output', type=Path,
                        default=ROOT / 'reports/security-benchmark-v1/binding-preflight-01')
    ...
    write_json(args.output / 'call-envelope.json', {
        key: result[key] for key in (
            'schema_version', 'protocol_sha256', 'binding_sha256', 'billing_unit',
    ...
    unknown = ', '.join(row['role_id'] for row in result['unresolved_cost_roles'])
    lines = [
        '# #112 真实绑定与零调用预算冻结', '',
        ...
        f'- 已知绑定费用：{result["known_bound_cost"]:.6f} {result["billing_unit"]}',
        f'- 未知费用角色：{unknown or "无"}',
    ...
    request = [
        ...
        f'- 完整费用上限：{"尚不可计算" if result["maximum_total_cost"] is None else str(result["maximum_total_cost"]) + " " + result["billing_unit"]}。', '',
        '## 尚未满足', '',
        *[f'- {item}' for item in result['blocking_requirements']], '',
        '## 授权状态', '',
