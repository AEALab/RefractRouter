...
def run_agent(payload, *, mode='preflight', runs_dir, production_budget=40,
    ...
    profile = json.loads(Path(profile_path).read_text() if profile_path else resource('report-profile.json').read_text())
    ...
    result = run_task(request, manifest, profile,
        client=(client or OpenAICompatibleClient(max_retries=0)) if mode == 'live' else None,
