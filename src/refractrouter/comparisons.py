...
def paired_comparisons(observations, *, expected_blocks=None, comparison_pairs=None, interpretation=None):
    indexed = {}
    ...
    pairs = []
    summaries = {}
    requested = comparison_pairs if comparison_pairs is not None else tuple(
        (candidate, baseline) for candidate in ("node-oracle", "node-type-rule")
        for baseline in ("strong-all", "task-oracle"))
    for candidate, baseline in requested:
        rows = []
    ...
    return {
        ...
        "interpretation": interpretation or "Descriptive paired differences; repeats are nested within tasks. "
